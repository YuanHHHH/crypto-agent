...
coin_quest=input("请输入你想询问的代币\n")
coin_price = get_crypto_price(coin_quest)
data = {
    "model":"MiniMax-M2.7",
    "messages": [
        {"role": "system", "content": "你是一个加密货币数据分析助手。你的任务是根据提供的价格数据进行客观的技术分析，包括趋势判断、波动率分析、关键价位识别。你只做数据层面的分析，不提供买卖建议。"},
...
while True:
    ...
    new_user_input = input("请输入你想问的问题")
    if new_user_input=="没有了":
        break
    data["messages"].append(
        {
            "role": "user",
