...
加密货币 AI 分析 Agent，支持实时价格查询、市场数据分析、AI 智能行情分析、ReAct Agent 自主工具调用、多轮对话记忆、RAG 知识库检索、推理过程可视化。基于 MiniMax LLM + Streamlit + Chroma 构建。
...
- RAG 知识库检索：10 篇加密货币概念文档，Chroma 向量数据库，语义检索
- Agent 智能路由：概念类问题走知识库检索，实时数据走 API，复合问题串行调用
...
- LangChain 版 Agent：create_react_agent + AgentExecutor + ConversationBufferMemory + TraceCallback
...
这会读取 `data/docs/` 下的 10 篇加密概念文档，切分、embedding、存入 Chroma。
...
交互式检索知识库，输入 query 返回 top-5 相关 chunk 及距离分数。
...
crypto-agent/
    ...
    docs/
        screenshot.png          # 界面截图
        custom_vs_langchain.md  # 手写 vs LangChain 对比分析
        week7_notes.md          # Week 7 学习笔记
        week8_notes.md          # Week 8 学习笔记
    scripts/
        batch_test.py           # Agent 批量测试脚本
        build_rag_index.py      # RAG 知识库建库脚本
    src/
        ...
        agent/
            ...
            prompts.py          # Agent system prompt（含 RAG 路由规则）
            trace.py            # 运行轨迹记录
            eval.py             # Agent 质量评估
            langchain_agent.py  # LangChain 版 Agent 入口
            langchain_tools.py  # @tool 包装 + sanitize
            langchain_callbacks.py  # TraceCallback 实现
        rag/
            chunk.py            # 文本切分（RecursiveCharacterTextSplitter）
            embed.py            # Embedding 封装（all-MiniLM-L6-v2）
            vector_store.py     # VectorStore 类（Chroma 封装）
            cli.py              # 检索 CLI 工具
        tools/
    ...
    data/
        ...
        docs/                   # RAG 知识库语料（10 篇加密概念文档）
        vector_db/              # Chroma 向量数据库（.gitignore）
        traces/
...
- LangChain 0.3.28（langchain-core、langchain-openai）
- ChromaDB（本地向量数据库）
...
拼接 chat_history + 用户问题    ← 多轮对话记忆
  ↓
构建 system prompt（含工具描述 + RAG 路由规则）
  ↓
while 循环（max_steps=5）：
  ├→ 发送 conversation 给 LLM
  ├→ parse_llm_output 解析为结构化 dict
  ├→ type == "action"       → 调用 ToolRegistry.call() → 追加 Observation
  │   ├→ get_price / get_market / get_coin_detail / analyze_coin（API 工具）
  │   └→ search_rag_knowledge（RAG 检索工具）
...
- **VectorStore**：Chroma 向量数据库封装，支持 add / search / count / reset，embedding 使用 all-MiniLM-L6-v2
...
- `search_rag_knowledge`：RAG 知识库检索（查询加密货币概念、原理、对比等知识类问题）
...
- Week 7: LangChain 重构（create_react_agent + AgentExecutor + Memory + Callbacks + 手写 vs 框架对比）
- Week 8: RAG 基础（chunk + embedding + Chroma + 知识库检索 + Agent 路由）
...
本周为 crypto-agent 增加了 RAG（Retrieval Augmented Generation）能力，Agent 从「只能调 API 查实时数据」升级为「能检索知识库回答概念问题 + 调 API 查实时数据」的双模态 Agent。

### 知识库

- 10 篇加密货币概念文档（data/docs/），涵盖 Bitcoin、Ethereum、PoS、PoW、DeFi、NFT、DEX、Stablecoin、Smart Contract、Solana
- 使用 RecursiveCharacterTextSplitter 切分（chunk_size=500, overlap=50）
- 使用 all-MiniLM-L6-v2 做 embedding（384 维）
- 使用 Chroma 本地向量数据库存储和检索

### RAG 工具

新增第 5 个工具 `search_rag_knowledge`，Agent 根据问题类型自动路由：

- 概念类问题（什么是 X、X 的原理）→ search_rag_knowledge
- 实时数据问题（价格、市值）→ get_price / get_market
- 复合问题（技术原理 + 价格）→ 先检索再查 API

### 路由测试结果

- 单工具路由：100% 正确
- 复合路由：工具选择正确，能串行调用不同工具
- 已知问题：LLM 偶尔编造 Observation 绕过工具调用（MiniMax 格式遵从度问题）

## 后续计划

- Week 9: LangGraph 状态图 Agent + LangChain 1.x 迁移
- Week 10-12: RAG + Agent 深度集成、工程优化、Portfolio v1.0
- Week 13-24: 加密货币领域专项、面试准备
